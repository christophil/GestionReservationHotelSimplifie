...
public class ModelRecherche{
  ...
  public static int getNumeroClient(String nom, String prenom){

      int resultat = 0;

      try{

        PreparedStatement requete = ConnexionBDDProjet.getInstance().prepareStatement("SELECT id FROM Client WHERE prenom = ? AND nom = ?");
        ...
        try{
          ...
          if(numeroClient.next() == false){

            return resultat;
          }

          resultat = numeroClient.getInt(1);
  ...
  public static int getNombreReservation(int numeroClient){

      int resultat = 0;

      try{

        PreparedStatement requete = ConnexionBDDProjet.getInstance().prepareStatement("SELECT COUNT(*) FROM Reservation WHERE client = ?");
  ...
  public static void choisirChambre(int numeroClient, int numeroChambre, String date){
    ...
    try{
      ...
      PreparedStatement requete2 = ConnexionBDDPerso.getInstance().prepareStatement("INSERT INTO historique(date, client, chambre) VALUES(?, ?, ?)");
      requete2.setString(1, date);
      requete2.setInt(2, numeroClient);
      requete2.setInt(3, numeroChambre);
